package com.example.weofapphao.czm.ui.View;
...
public class Sprite {
    ...
    public void setDirection() {
        if (this.x <= 0
                && (this.y + this.height) < GameView.SCREEN_HEIGHT) {
            if (this.x < 0)
                this.x = 0;
            this.direction = Sprite.DOWN;
        } else if ((this.y + this.height) >= GameView.SCREEN_HEIGHT
                && (this.x + this.width) < GameView.SCREEN_WIDTH) {
            if ((this.y + this.height) > GameView.SCREEN_HEIGHT)
                this.y = GameView.SCREEN_HEIGHT - this.height;
            this.direction = Sprite.RIGHT;
        } else if ((this.x + this.width) >= GameView.SCREEN_WIDTH
                && this.y > 0) {
            if ((this.x + this.width) > GameView.SCREEN_WIDTH)
                this.x = GameView.SCREEN_WIDTH - this.width;
